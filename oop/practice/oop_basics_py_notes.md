...
Variables are called "variables" because they can hold any value and that _value can change_ (it varies).
...
Conventionally this _"entry point"_ function is usually called main to keep things simple and consistent.
...
_Parameters_ are the names used for inputs when _defining a function_. _Arguments_ are the values of the inputs supplied when a _function is called_.
...
Similarly to checking values within an array or a string we use _in_ and _not in_
...
- proper way of creating classes is by using a _constructor_ witch in python is a special method **init**
...
_Abstraction_ is about creating a simple interface to achieve a complex behavior. It focuses on what is exposed
...
_Encapsulation_ is about hiding internal state
...
We should not overuse inheritance. Generally _we should make class inherit from other class only if they share all
parent class properties / methods._
...
be a _reference to the original list..._ which is not what we want.
...
_23_polymorphism for reference_
...
The **repr** method works similarly: the difference is that it's intended for use in debugging by developers, rather than in printing strings to end users.
