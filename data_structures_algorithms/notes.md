...
Order log(n) - `złożoność logarytmiczna`
Order log(n) are only slightly slower than log(1). They grow according to the input size but to the log of the input.
...
Binary search is the common example of log(n) input. It works on a pre-sorted list.
